...
export function GlassEffect({ children, className = "", style = {}, href, target = "_blank", onClick }) {
  const glassStyle = {
  ...
  const content = (
    ...
    >
      ...
      <div
        ...
        style={{ background: "rgba(255,255,255,0.08)", borderRadius: "inherit" }}
      />
      <div
        className="absolute inset-0 z-20 rounded-3xl overflow-hidden"
        style={{ boxShadow: "inset 2px 2px 1px 0 rgba(255,255,255,0.3), inset -1px -1px 1px 1px rgba(255,255,255,0.15)" }}
      />
...
export function GlassCard({ children, className = "", onClick }) {
  return (
    <GlassEffect className={`rounded-2xl p-6 ${className}`} onClick={onClick}>
      {children}
    </GlassEffect>
  );
}
